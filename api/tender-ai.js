...
const tenderStore = require("../lib/nexus/tender-store");
const { searchTenders, TENDER_SOURCES } = require("../lib/nexus/tender-ai");
...
async function post(req, res, action, session) {
  ...
  return json(res, 201, {
    ok: true,
    data: await tenderStore.saveSearch(result, session.email)
  });
}
...
module.exports = async function handler(req, res) {
  ...
  if (!session) return;
  const url = new URL(req.url || "/api/tender-ai", "http://localhost");
