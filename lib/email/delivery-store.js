...
async function recordAccepted(data) {
  ...
  if (saved) await crmStore.syncEmailDelivery({
    recipient: saved.recipient, subject: saved.subject,
    providerMessageId: saved.provider_message_id, status: saved.status
  });
  return saved;
}
...
async function recordWebhookEvent(event, eventId) {
  ...
  const result = await query(`
    ...
    RETURNING *`, [
      clean(data.from, 320) || clean(emailConfig().from, 320) || "unknown",
...
async function summary() {
  const result = await query(`
    SELECT count(*)::int total,
      count(*) FILTER (WHERE status IN ('delivered','opened','clicked'))::int delivered,
