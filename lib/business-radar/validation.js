...
function opportunity(input = {}) {
  const reference = clean(input.reference || input.externalId, 200);
  const data = {
    ...
    sourceUrl: clean(input.sourceUrl, 1200), externalId: reference, publishedAt: optionalDate(input.publishedAt),
