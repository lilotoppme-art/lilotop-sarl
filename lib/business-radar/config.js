...
function radarConfig() {
  const previewDatabaseUrl = clean(process.env.DATABASE_URL_PREVIEW);
  const isPreview = clean(process.env.VERCEL_ENV) === "preview";
  return {
    databaseUrl: isPreview && previewDatabaseUrl ? previewDatabaseUrl : clean(process.env.DATABASE_URL),
