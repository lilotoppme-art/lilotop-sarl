...
const crmStore = require("../nexus/crm-store");
const { scanMonitoredItbs } = require("../nexus/itb-monitor");
...
async function prepare(raw, sourceId = null) {
  ...
  let analysis;
  try { analysis = await analyzeOpportunity(base); } catch (error) {
    const diagnostic = String(error.message || error.code || "AI_ANALYSIS_FAILED").slice(0, 160);
    analysis = {
      mode: "no_ai_fallback",
      summary: `${data.title} - analyse IA indisponible`,
      fit: "a_verifier",
      risks: [error.code || "AI_ANALYSIS_FAILED", diagnostic],
      nextActions: ["Reprendre l'analyse manuellement"],
    };
  }
...
async function saveOpportunity(raw, sourceId = null, options = {}) {
  const saved = await store.upsertOpportunity(await prepare(raw, sourceId));
  await crmStore.syncOpportunity(saved);
  if (saved.is_demo || options.intake === false) return saved;
  const { intakeOpportunity } = require("../nexus/opportunity-intake");
  const intake = await intakeOpportunity(saved, options.intakeOptions || {});
  return { ...saved, intake: intake.intake, workflowId: intake.workflow?.id || null };
}
...
async function runRadar(triggerType = "manual") {
  ...
  try {
    ...
    try {
      stats.itbMonitoring = await scanMonitoredItbs();
    } catch (error) {
      stats.status = "partial";
      stats.errors.push({ code: error.code || "ITB_MONITOR_FAILED", message: String(error.message).slice(0, 240) });
    }
    try {
      const { runAutomaticPipelineCycle } = require("../nexus/pipeline-runner");
      stats.pipeline = await runAutomaticPipelineCycle();
    } catch (error) {
      stats.status = "partial";
      stats.errors.push({ code: error.code || "PIPELINE_CYCLE_FAILED", message: String(error.message).slice(0, 240) });
    }
  } catch (error) {
    stats.status = "failed";
  ...
  return { ...finished, itbMonitoring: stats.itbMonitoring || null, pipeline: stats.pipeline || null };
}
