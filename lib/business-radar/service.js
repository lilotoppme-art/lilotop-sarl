...
const { sendOpportunityAlert } = require("./notifications");
...
async function runRadar(triggerType = "manual") {
    ...
    for (const source of sources) {
      try {
        ...
        for (const item of items) {
          ...
          if (saved.inserted && saved.score >= 70 && !saved.is_demo) await sendOpportunityAlert(saved);
        }
  ...
  return store.finishRun(run.id, stats);
}
