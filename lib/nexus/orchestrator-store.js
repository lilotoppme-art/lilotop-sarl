...
async function listOpportunities(limit = 30) {
  const result = await query(`
    SELECT id, external_id, status, is_demo, title, organization, country, sector, opportunity_type,
      description, source_url, deadline_at, estimated_value, currency,
      score, ai_summary, ai_analysis, analysis_mode, raw_data, created_at, updated_at
    FROM opportunities
    WHERE (
...
async function listPipelineOpportunities(limit = 100) {
  const result = await query(`
    SELECT id, external_id, status, is_demo, title, organization, country, sector, opportunity_type,
      description, source_url, deadline_at, estimated_value, currency,
      score, ai_summary, ai_analysis, analysis_mode, raw_data, created_at, updated_at
  ...
  return listOpportunitiesFromRows(result.rows);
}
...
async function createWorkflow(opportunity, actorEmail) {
  return transaction(async (client) => {
    await client.query("SELECT pg_advisory_xact_lock(hashtext($1))", [String(opportunity.id)]);
    const existing = await client.query(`
      ...
      LIMIT 1
    `, [opportunity.id]);
    if (existing.rowCount) return mapWorkflow(existing.rows[0]);
    const dossier = {
      opportunity,
...
async function countIntensiveWorkflows(excludeId = null) {
  const result = await query(`SELECT count(*)::int AS count FROM nexus_workflows
    WHERE status='running' AND ($1::uuid IS NULL OR id <> $1::uuid)`, [excludeId]);
...
async function dashboardSummary() {
  const [result, recent, emailTracking, opportunities, workflows] = await Promise.all([query(`
    SELECT
  ...
  `), query(`
    ...
    ORDER BY updated_at DESC
    LIMIT 8
  `), emailTrackingSummary(), listPipelineOpportunities(100), listWorkflows(100)]);
  const row = result.rows[0] || {};
  const pipeline = buildPipelineBoard(opportunities, workflows);
  return {
