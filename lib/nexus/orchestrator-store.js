...
async function listOpportunities(limit = 30) {
  const result = await query(`
    ...
    FROM opportunities
    WHERE (is_demo = false OR (external_id = 'EOIUNECA24536' AND source_url = 'https://www.ungm.org/Public/Notice/306489'))
...
async function getOpportunity(id) {
  const result = await query(`
    ...
    FROM opportunities
    WHERE id = $1
      AND (is_demo = false OR (external_id = 'EOIUNECA24536' AND source_url = 'https://www.ungm.org/Public/Notice/306489'))
    LIMIT 1
  `, [id]);
