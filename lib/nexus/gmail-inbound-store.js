...
async function excludeOutboundMessages() {
  await ensureSchema();
  const result = await query(`UPDATE nexus_supplier_inbound_messages AS inbound
    SET rfq_id=NULL,supplier=NULL,lot_number=NULL,matching_status='MESSAGE SORTANT IGNORE',
      extraction='{}'::jsonb
    FROM nexus_supplier_outbound_authorizations AS outbound
    WHERE outbound.provider_message_id IS NOT NULL
      AND inbound.gmail_message_id=outbound.provider_message_id
      AND (inbound.rfq_id IS NOT NULL OR inbound.matching_status <> 'MESSAGE SORTANT IGNORE')`);
  return result.rowCount || 0;
}
async function excludeInternalMessages(mailboxes) {
  await ensureSchema();
  const patterns = [...new Set((mailboxes || []).filter(Boolean).map((value) => `%${String(value).toLowerCase()}%`))];
  if (!patterns.length) return 0;
  const result = await query(`UPDATE nexus_supplier_inbound_messages
    SET rfq_id=NULL,supplier=NULL,lot_number=NULL,matching_status='MESSAGE INTERNE IGNORE',
      extraction='{}'::jsonb
    WHERE lower(sender) LIKE ANY($1::text[])
      AND (rfq_id IS NOT NULL OR matching_status <> 'MESSAGE INTERNE IGNORE')`, [patterns]);
  return result.rowCount || 0;
}
...
async function listMatchedInbound(workflowId) {
  await ensureSchema();
  const result = await query(`SELECT gmail_message_id,gmail_thread_id,message_id_header,in_reply_to,
    references_header,sender,recipient,reply_to,subject,received_at,rfq_id,workflow_id,supplier,
    lot_number,matching_status,body_text,attachments,extraction,created_at
    FROM nexus_supplier_inbound_messages
    WHERE workflow_id=$1 AND rfq_id IS NOT NULL
      AND rfq_id <> 'NEXUS-INTERNAL-EMAIL-ROUTING-TEST'
      AND matching_status='REPONSE RATTACHEE'
    ORDER BY received_at ASC,created_at ASC`, [workflowId]);
  return result.rows;
}
module.exports = { archiveInboundMessage, authorizeOutbound, connectionStatus, ensureSchema, excludeInternalMessages, excludeOutboundMessages, knownMessageIds, latestInboundByRfq, listMatchedInbound, loadConnection, markOutboundFailed, markOutboundSent, markSynced, outboundAuthorization, saveConnection };
