...
- DAP Lilongwe, Malawi - Incoterms 2020 price, with transport identified separately where available;
...
function buildHiltiPilot(rfqs, env = process.env) {
  ...
  const fromConfigured = Boolean(env.RESEND_API_KEY && env.EMAIL_FROM);
  const inboundConfigured = Boolean(
    (env.GMAIL_OAUTH_CLIENT_ID && env.GMAIL_OAUTH_CLIENT_SECRET && env.GMAIL_OAUTH_REFRESH_TOKEN)
    || (env.RESEND_INBOUND_WEBHOOK_SECRET && env.RESEND_RECEIVING_DOMAIN)
  );
  return {
    rfqId: rfq.id,
    supplier: "Hilti",
    ...
    dryRun: {
      ...
      sender: fromConfigured ? String(env.EMAIL_FROM).replace(/<[^>]+>/, "<adresse configuree>") : "NON CONFIGURE DANS CET ENVIRONNEMENT",
    ...
    responseTracking: {
      operational: inboundConfigured,
      manualEvidenceIntakeReady: true,
      automaticEmailDetectionReady: inboundConfigured,
      archiveOriginalReady: inboundConfigured,
      archiveAttachmentsReady: inboundConfigured,
      quotationExtractionReady: true,
      missingDataPolicy: "NE JAMAIS INVENTER - conserver EN ATTENTE si absent",
      blocker: inboundConfigured ? null : "Aucun connecteur Gmail OAuth actif ni domaine Resend Inbound confirme pour detecter automatiquement les reponses fournisseurs."
    },
    authorization: {
      pilotOnly: true,
...
function buildRfq(lot, supplier, products, existing, preparedAt) {
  ...
  const rfq = {
    ...
    attachments: supplier === "Hilti"
      ? [HILTI_PILOT_ATTACHMENT]
      : ["Schedule of Requirements officiel du lot", "Tableau technique du lot"],
    coverageCounts,
    hasInventedData,
