...
function buildReplacementRfq(lot, definition, products, existing, preparedAt) {
  ...
  const authorizedForIndividualReview = definition.dgSendAuthorized === true;
  return {
    ...base,
    priority: "REMPLACEMENT",
    status: existing?.status || "PREPAREE - NON AUTORISEE - VALIDATION DG REQUISE",
    sendRecommendation: authorizedForIndividualReview ? "OUI" : "NON - NOUVELLE AUTORISATION DG REQUISE",
    ...
    authorizationConfirmation: {
      ...
      blockedUntilNewDgAuthorization: !authorizedForIndividualReview
    }
  };
}
