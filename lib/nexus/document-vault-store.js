...
function mapRow(row, includeFile = false) {
  ...
  const mapped = {
    ...
    status: row.expires_on && new Date(row.expires_on) < new Date(new Date().toISOString().slice(0, 10))
      ? "expired"
      : "valid"
  };
  ...
  return transaction(async (client) => {
    ...
    if (documentId) {
      ...
      const created = await client.query(`
        INSERT INTO document_vault_documents (title, category, description, created_by)
        VALUES ($1, $2, $3, $4)
        RETURNING id
      `, [metadata.title, metadata.category, metadata.description, uploadedBy]);
