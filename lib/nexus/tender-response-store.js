...
async function saveAnalysis(result, createdBy) {
  const status = ["draft", "validated", "archived"].includes(result.status)
    ? result.status
    : "draft";
...
async function saveRevision(base, changes, createdBy) {
  const workflow = base.keyInformation?.workflow || {};
  const revised = {
    ...base,
    ...changes,
    keyInformation: {
      ...base.keyInformation,
      ...(changes.keyInformation || {}),
      workflow: {
        ...workflow,
        ...(changes.workflow || {}),
        version: Math.max(1, Number(workflow.version) || 1) + 1
      }
    },
    generatedDocuments: {
      ...base.generatedDocuments,
      ...(changes.generatedDocuments || {})
    }
  };
  return saveAnalysis(revised, createdBy);
}
