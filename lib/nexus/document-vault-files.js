...
function documentRole(text) {
  ...
  if (/\b(bon de livraison|delivery note)\b/i.test(text)) return "delivery_note";
  if (/\b(attestation de r[ée]ception|acceptance certificate)\b/i.test(text)) return "acceptance_certificate";
  if (/\b(bonne ex[ée]cution|performance certificate|completion certificate)\b/i.test(text)) return "performance_certificate";
  return "";
}
...
async function extractScannedPdfMetadata(file, options = {}) {
  ...
  const fields = [
    "documentType", "clientAuthority", "reference", "date", "subject", "supplier",
    "products", "quantities", "amount", "currency", "deliveryPlace", "incoterm",
    "leadTime", "signaturesOrStamps"
  ];
...
function analyzeVaultDocument(file, input = {}, extracted = {}) {
  const detectedContent = [
    extracted.documentType, extracted.clientAuthority, extracted.reference, extracted.date,
    ...
    extracted.leadTime, extracted.signaturesOrStamps
  ].filter(Boolean).join("\n");
  ...
  const reference = extracted.reference || valueAfterLabel(content, [
  ...
  const authority = extracted.clientAuthority || valueAfterLabel(content, [
  ...
  const subject = extracted.subject || valueAfterLabel(content, ["objet", "subject", "description", "purpose"]);
  const client = extracted.clientAuthority || valueAfterLabel(content, ["client", "customer", "acheteur", "buyer"]);
  const role = documentRole(searchable);
  const experience = isExperience ? {
    ...
    productsServices: inputOrDetected(input, "experienceProducts", extracted.products || valueAfterLabel(content, ["produits", "products", "services", "fournitures", "goods"])),
    quantities: inputOrDetected(input, "experienceQuantities", extracted.quantities || valueAfterLabel(content, ["quantités", "quantities", "quantity", "qty"])),
    contractNumber: reference,
    date: extracted.date || issueDates[0] || dates[0] || "",
  ...
  return {
    ...
    documentType: extracted.documentType || businessDocumentType(searchable),
    fileFormat: file.extension.toUpperCase(),
    version: detectedVersion(searchable),
    reference,
    issuingAuthority: authority,
    issuedOn: extracted.date || issueDates[0] || dates[0] || "",
    expiresOn: expiryDates[0] || "",
    description: detectedDescription(content),
    notes: valueAfterLabel(content, ["notes", "note", "remarques", "remarks"]),
    source: input.source || "Import DG",
    needsConfirmation: !reference || !authority || !businessDocumentType(searchable)
