...
function analyzeVaultDocument(file, input = {}) {
  ...
  const subject = valueAfterLabel(content, ["objet", "subject", "description"]);
  const client = valueAfterLabel(content, ["client", "customer", "acheteur", "buyer"]);
  ...
  const experience = isExperience ? {
    ...
    productsServices: inputOrDetected(input, "experienceProducts", valueAfterLabel(content, ["produits", "products", "services", "fournitures", "goods"])),
    quantities: inputOrDetected(input, "experienceQuantities", valueAfterLabel(content, ["quantités", "quantities", "quantity", "qty"])),
    ...
    value: inputOrDetected(input, "experienceValue", valueAfterLabel(content, ["valeur", "value", "montant", "amount"])),
    currency: inputOrDetected(input, "experienceCurrency", (searchable.match(/\b(USD|EUR|CDF|ZAR|GBP|MWK)\b/i)?.[1] || "")).toUpperCase(),
    country: inputOrDetected(input, "experienceCountry", valueAfterLabel(content, ["pays", "country"])),
    deliveryPlace: inputOrDetected(input, "experienceDeliveryPlace", valueAfterLabel(content, ["lieu de livraison", "delivery place", "delivery location", "destination"])),
    incoterm: inputOrDetected(input, "experienceIncoterm", searchable.match(/\b(EXW|FCA|CPT|CIP|DAP|DPU|DDP|FAS|FOB|CFR|CIF)\b/i)?.[1]?.toUpperCase()),
    leadTime: inputOrDetected(input, "experienceLeadTime", valueAfterLabel(content, ["délai", "lead time", "delivery period"])),
