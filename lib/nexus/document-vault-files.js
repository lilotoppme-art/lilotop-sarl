...
function proposeExperienceAssociation(analysis, documents = []) {
  ...
  const analysisReference = normalizedMatchValue(
    analysis.experience.groupReference || analysis.experience.contractNumber || analysis.reference
  );
  const candidates = documents.filter((item) => item.experience).map((item) => {
    const experience = item.experience || {};
    const candidateReference = experience.group_reference || experience.contract_number
      || experience.client_reference || item.reference || "";
    const normalizedReference = normalizedMatchValue(candidateReference);
    const referenceMatch = Boolean(analysisReference && normalizedReference
      && (analysisReference.includes(normalizedReference) || normalizedReference.includes(analysisReference)));
    ...
    const score = referenceMatch ? 1 : (clientMatch * 0.45) + (productMatch * 0.55);
  ...
  return {
    ...
    confidence: best.referenceMatch || best.score >= 0.75 ? "ÉLEVÉE" : best.score >= 0.45 ? "MOYENNE" : "FAIBLE",
