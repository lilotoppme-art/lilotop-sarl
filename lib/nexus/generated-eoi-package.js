...
function buildUnecaEoiArtifacts(submission) {
  const items = [
    ...
    { text: "3. ELIGIBILITY DECLARATIONS A-G", bold: true, size: 13, before: 10, after: 6 },
    ...submission.eligibility.map((item) => ({ text: `${item.key}. ${item.declarationDraft} Status: ${item.status}.`, after: 5 })),
  ...
  const declarations = textSection("ELIGIBILITY DECLARATIONS A-G", submission.eligibility.map((item) => `${item.key}. ${item.declarationDraft}\nStatus: ${item.status}`));
  ...
  zip.addFile("03-Eligibility-Declarations-A-G.txt", Buffer.from(declarations, "utf8"));
