...
const UNOPS_LOTS = Object.freeze([
  ...
  {
    lot: 2,
    label: "Electrical Installation Components",
    pattern: /electri|câble|cable|disjoncteur|breaker|luminaire|éclairage|lighting|switchgear/i,
    exclusionPattern: /(?:appareil|instrument|equipment).{0,40}(?:mesure|measurement|test)|(?:mesure|measurement|test).{0,40}(?:électri|electri)/i,
    exclusionReason: "appareil de mesure ou de test, distinct des composants d'installation électrique du lot"
  },
...
function isExecutionProof(document) {
  const role = document.extractedMetadata?.experience?.documentRole || "";
  return ["delivery_note", "acceptance_certificate", "performance_certificate"].includes(role)
    || /^(BON DE LIVRAISON|PV DE R[ÉE]CEPTION|ATTESTATION DE BONNE EX[ÉE]CUTION)$/i
      .test(document.documentType || "");
}
...
function evaluateExperience(document, lot) {
  ...
  const delivered = experience.delivery_proof_available === true
    || /livr|deliver|achev|complet|success/i.test(experience.execution_status || document.previewText || "");
  const technicalExclusion = Boolean(lot.exclusionPattern?.test(searchable));
  const productMatch = lot.pattern.test(searchable) && !technicalExclusion;
  const realFile = document.filePresent === true;
  const validated = experience.dg_validated === true
    || Boolean(document.validatedBy && document.validatedAt);
  ...
  if (!productMatch) reasons.push(technicalExclusion
    ? lot.exclusionReason : "objet du document non compatible avec le lot");
...
function buildUnopsExperienceAudit(documents = []) {
  const experiences = documents.filter((item) => item.categoryCode === "04-experience-references"
    && item.experience && !isExecutionProof(item));
...
module.exports = { UNOPS_LOTS, buildUnopsExperienceAudit, evaluateExperience, isExecutionProof };
