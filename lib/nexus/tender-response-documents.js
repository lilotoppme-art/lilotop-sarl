...
async function readRawBody(req) {
  ...
  for await (const chunk of req) {
    ...
    if (size > MAX_REQUEST_BYTES) {
      throw error("Le fichier dépasse la taille maximale autorisée de 3 Mo.", "UPLOAD_TOO_LARGE");
    }
    chunks.push(buffer);
  }
...
async function parseUploadRequest(req) {
  ...
  if (!file.buffer.length || file.buffer.length > MAX_UPLOAD_BYTES) {
    throw error("Le fichier est vide ou dépasse la taille maximale autorisée de 3 Mo.", "UPLOAD_TOO_LARGE");
  }
