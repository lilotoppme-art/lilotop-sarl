...
function isInactiveSignal(signal, now = new Date()) {
  const context = [
    signal.timing,
  ...
  if (/\b(annule|cancelled|canceled|expire|expired)\b/.test(context)) {
    return true;
  }

  const closingDate = context.match(
    /\b(?:cloture|closing date|deadline|date limite)\D{0,24}(\d{4}-\d{2}-\d{2})\b/
  );
  if (!closingDate) return false;
  const closingEnd = Date.parse(`${closingDate[1]}T23:59:59.999Z`);
  return Number.isFinite(closingEnd) && closingEnd < now.getTime();
}
...
async function searchMiningSignals(input, options = {}) {
  ...
  const normalizedSignals = Array.isArray(result.signals)
    ? result.signals
      .map(normalizeSignal)
      .filter((signal) => signal.title && signal.company && signal.sourceUrl)
    : [];
  const signals = normalizedSignals
      .filter((signal) => !isInactiveSignal(signal, options.now))
      .sort((left, right) => right.opportunityScore - left.opportunityScore)
      .slice(0, 4);
  const excludedCount = normalizedSignals.length - signals.length;
  const exclusionNote = excludedCount
    ? ` ${excludedCount} signal(s) expire(s), annule(s) ou cloture(s) ont ete exclus automatiquement.`
    : "";
  return {
    mode: "openai",
    model: config.openaiModel,
    criteria,
    watchSummary: cleanText(`${result.watchSummary}${exclusionNote}`, 5000),
