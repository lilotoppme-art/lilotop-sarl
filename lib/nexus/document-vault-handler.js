...
const { analyzeVaultDocument, parseVaultUpload, prepareVaultFile, sanitizeFilename } = require("./document-vault-files");
...
async function post(req, res, action, session) {
  ...
  const analysis = analyzeVaultDocument(prepared, upload.fields);
  if (action === "analyze") {
