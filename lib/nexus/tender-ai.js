...
function isExpiredTender(tender, now = new Date()) {
  const context = [tender.deadline, tender.summary, tender.evidence]
    ...
    .toLowerCase();
  if (
    /\b(expire|expired|deadline (?:is )?passed|date limite (?:est )?depassee|annule|cancelled|canceled)\b/
      .test(context)
  ) {
    return true;
  }

  const deadline = cleanText(tender.deadline, 40)
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase();
  const isoMatch = deadline.match(/\d{4}-\d{2}-\d{2}/);
  let deadlineEnd = isoMatch
    ? Date.parse(`${isoMatch[0]}T23:59:59.999Z`)
    : NaN;

  if (!Number.isFinite(deadlineEnd)) {
    const monthNumbers = {
      janvier: 0,
      january: 0,
      fevrier: 1,
    ...
    const writtenMatch = deadline.match(
    ...
    if (writtenMatch) {
      deadlineEnd = Date.UTC(
        Number(writtenMatch[3]),
        monthNumbers[writtenMatch[2]],
  ...
  return Number.isFinite(deadlineEnd) && deadlineEnd < now.getTime();
}
    ...
    input: [
      {
        ...
        content: [
          ...
          "Retourne au maximum quatre appels d'offres, classés par score d'intérêt décroissant."
        ].join(" ")
      },
...
async function searchTenders(input, options = {}) {
  ...
  const normalizedTenders = Array.isArray(result.tenders)
    ? result.tenders.map(normalizeTender).filter((tender) => tender.title && tender.sourceUrl)
    : [];
  const tenders = normalizedTenders
    .filter((tender) => !isExpiredTender(tender, options.now))
    .sort((left, right) => right.interestScore - left.interestScore)
    .slice(0, 4);
  const expiredCount = normalizedTenders.length - tenders.length;
  const expiryNote = expiredCount
    ? ` ${expiredCount} avis expiré(s) ou hors périmètre ont été exclus automatiquement.`
    : "";
  return {
    mode: "openai",
    model: config.openaiModel,
    criteria,
    executiveSummary: cleanText(`${result.executiveSummary}${expiryNote}`, 5000),
