...
const OFFICIAL_HOSTS = new Set([
  ...
  "ecobank.com", "www.ecobank.com", "tala-com.com", "www.tala-com.com"
]);

function allowedOfficialHosts() {
  const configured = String(process.env.TENDER_SOURCE_ALLOWED_HOSTS || "")
    .split(",")
    .map((host) => host.trim().toLowerCase())
    .filter((host) => /^[a-z0-9.-]+$/.test(host));
  return new Set([...OFFICIAL_HOSTS, ...configured]);
}
...
function officialUrl(value) {
  ...
  if (url.protocol !== "https:" || !allowedOfficialHosts().has(url.hostname.toLowerCase())) {
...
function filenameFor(url, contentType) {
  ...
  if (/\.(pdf|docx|xlsx|zip)$/i.test(fromUrl)) return fromUrl;
  ...
  if (String(contentType).includes("spreadsheetml")) return `${fromUrl || "dao"}.xlsx`;
  if (String(contentType).includes("zip")) return `${fromUrl || "dao"}.zip`;
  throw sourceError("Le document officiel n'est ni un PDF, ni un DOCX, ni un XLSX, ni un ZIP.", "TENDER_SOURCE_UNSUPPORTED");
}
...
module.exports = { MAX_DOCUMENT_BYTES, OFFICIAL_HOSTS, allowedOfficialHosts, officialDocumentUrls, officialUrl, retrieveOfficialDocument };
