...
function normalizeResult(result, input, model) {
  ...
  return {
    ...
    compliance: {
      ...
      expiredDocuments: cleanList(compliance.expiredDocuments, 50),
...
async function prepareTenderResponse(document, fields = {}, options = {}) {
  const vaultDocuments = Array.isArray(options.vaultDocuments)
    ? options.vaultDocuments.slice(0, 200)
    : [];
    ...
    input: [
      {
        ...
        content: [
          ...
          "Compare les documents exigés avec le coffre documentaire LILOTOP fourni et avec les éventuels documents complémentaires déclarés.",
          "Un document du coffre marqué expired doit figurer dans expiredDocuments et ne doit jamais être compté comme disponible.",
          "Un document exigé sans équivalent valide dans le coffre ou la liste complémentaire doit figurer dans missingDocuments.",
