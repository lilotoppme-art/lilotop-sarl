...
function buildDocumentControl(requiredDocuments = [], vaultDocuments = [], declaredDocuments = []) {
  ...
  const rows = catalog.map((item) => {
    const matches = vaultDocuments.filter((document) =>
      matchesDocument(documentEvidence(document), [item.label, ...item.aliases])
    );
    const available = matches.find((document) => document.usableInTenders === true);
    const expired = matches.find((document) =>
      document.filePresent === true
      && document.usableForTenders === true
      && document.status === "expired"
    );
    const declared = declaredDocuments.find((document) =>
      matchesDocument(document, [item.label, ...item.aliases])
    );
    const physicalFile = matches.find((candidate) => candidate.filePresent === true) || null;
    const document = available || expired || physicalFile || null;
    const status = available ? "available" : expired ? "expired" : "missing";
    return {
      key: item.key,
      document: item.label,
      status,
      ...
      matchingDocument: document?.title || null,
      storageLocation: document?.storageLocation || null,
      source: document
        ? `${document.sourceFilename || document.title} - ${document.storageLocation || "Coffre documentaire"}`
        : declared
          ? "Déclaré sans fichier accessible - non utilisable"
          : "Non trouve",
      documentId: document?.id || null,
      versionId: document?.versionId || null,
      sourceFilename: document?.sourceFilename || null,
      filePresent: document?.filePresent === true,
      usableInTenders: document?.usableInTenders === true,
