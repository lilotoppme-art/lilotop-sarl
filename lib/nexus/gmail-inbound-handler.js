  ...
  try {
    ...
    const supplierCycle = {
      ...
      rfqs: workflow.dossier.supplierCycle.rfqs.map((item) => item.id === rfq.id
        ? { ...item, status: "RFQ ENVOYEE", trackingStatus: "SENT", emailSent: true, sentAt, gmailMessageId: sent.id, messageIdHeader: metadata.messageIdHeader }
        : item)
    };
...
async function syncSupplierReplies() {
  ...
  if (workflow) {
    ...
    const reconciliation = gmail.reconcileRfqTracking(currentCycle.rfqs || [], responses);
    const responseStats = {
    ...
    const rfqs = reconciliation.rfqs.map((rfq) => rfq.trackingStatus === "DELIVERY FAILED" ? {
      ...rfq,
      status: "DELIVERY FAILED",
      deliveryFailure: responses.find((item) => item.rfqId === rfq.id && item.responseType.startsWith("F.")) || rfq.deliveryFailure || null
    } : rfq);
    responseStats.effectivelyDelivered = responseStats.noBounceDetected;
    if (JSON.stringify(previousIds) !== JSON.stringify(nextIds)
      ...
      || JSON.stringify((currentCycle.rfqs || []).map((item) => [item.deliveryStatus || null, item.trackingStatus || null])) !== JSON.stringify(rfqs.map((item) => [item.deliveryStatus || null, item.trackingStatus || null]))) {
      const supplierCycle = {
        ...currentCycle, rfqs, responses, responseStats,
        counts: {
          ...currentCycle.counts,
          sent: sentCount,
          sendAttempts: responseStats.sendAttempts,
          uniqueRfqs: responseStats.uniqueRfqs,
          active: responseStats.activeRfqs,
          delivered: responseStats.noBounceDetected,
          deliveryFailed: responseStats.deliveryFailed,
          received: responses.length,
          missing: responseStats.withoutResponse
        }
      };
      const finalValidation = {
