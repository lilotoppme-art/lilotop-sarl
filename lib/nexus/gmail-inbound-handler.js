...
async function syncSupplierReplies() {
  ...
  if (workflow) {
    ...
    const deliveryFailures = responses.filter((item) => item.responseType.startsWith("F.")
      && item.recommendedAction === "CHERCHER FOURNISSEUR ALTERNATIF");
    const failedRfqIds = new Set(deliveryFailures.map((item) => item.rfqId));
    const rfqs = (currentCycle.rfqs || []).map((rfq) => failedRfqIds.has(rfq.id) ? {
      ...rfq,
      status: "DELIVERY FAILED / CONTACT A REMPLACER",
      deliveryStatus: "FAILED",
      deliveryFailure: responses.find((item) => item.rfqId === rfq.id && item.responseType.startsWith("F.")) || null
    } : rfq);
    responseStats.deliveryFailed = failedRfqIds.size;
    responseStats.effectivelyDelivered = Math.max(0, sentCount - failedRfqIds.size);
    if (JSON.stringify(previousIds) !== JSON.stringify(nextIds)
      ...
      || JSON.stringify((currentCycle.rfqs || []).map((item) => item.deliveryStatus || null)) !== JSON.stringify(rfqs.map((item) => item.deliveryStatus || null))) {
      const supplierCycle = {
        ...currentCycle, rfqs, responses, responseStats,
        counts: {
          ...currentCycle.counts,
          sent: sentCount,
          delivered: responseStats.effectivelyDelivered,
