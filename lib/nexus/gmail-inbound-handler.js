...
const INTERNAL_SEND_TEST_RFQ = {
  id: "NEXUS-INTERNAL-GMAIL-SEND-TEST",
  trackingId: "NEXUS-RFQ-SEND-TEST",
  supplier: "LILOTOP INTERNAL SEND TEST",
  lotNumber: 0,
  contactEmail: "contact@lilotopsarl.com"
};
...
function assertPreview() {
  if (process.env.VERCEL_ENV !== "preview") {
    throw Object.assign(new Error("Internal Gmail send testing is Preview-only"), { code: "PREVIEW_ONLY" });
  }
}

function internalTestMessage() {
  const identity = outbound.assertIdentityArchitecture();
  return {
    from: identity.rfqFrom,
    to: "contact@lilotopsarl.com",
    replyTo: identity.rfqReplyTo,
    subject: `[TEST INTERNE NEXUS] ${INTERNAL_SEND_TEST_RFQ.trackingId} - Cotation fictive`,
    text: [
      "TEST INTERNE LILOTOP - NE PAS TRAITER",
      `RFQ: ${INTERNAL_SEND_TEST_RFQ.id}`,
      "Supplier: LILOTOP INTERNAL SEND TEST",
      "Currency: USD",
      "Unit price: 125.50",
      "Total price: 376.50",
      "Incoterm: DAP Kinshasa",
      "Lead time: 10 days",
      "This message validates Gmail API transport only."
    ].join("\n"),
    attachments: [{
      filename: "NEXUS_INTERNAL_TEST_QUOTATION.csv",
      contentType: "text/csv; charset=UTF-8",
      content: Buffer.from("item,quantity,unit_price,currency,total\nInternal test item,3,125.50,USD,376.50\n", "utf8")
    }]
  };
}

async function authorizeInternalSendTest(actorEmail) {
  assertPreview();
  outbound.assertIdentityArchitecture();
  return store.authorizeOutbound({
    rfqId: INTERNAL_SEND_TEST_RFQ.id,
    supplier: INTERNAL_SEND_TEST_RFQ.supplier,
    recipient: INTERNAL_SEND_TEST_RFQ.contactEmail,
    lotNumber: 0,
    authorizedBy: actorEmail,
    metadata: { internalTest: true, externalSupplier: false }
  });
}

async function sendInternalTest() {
  assertPreview();
  const authorization = await store.outboundAuthorization(INTERNAL_SEND_TEST_RFQ.id);
  if (!authorization || authorization.status !== "AUTHORIZED" || authorization.metadata?.internalTest !== true) {
    throw Object.assign(new Error("Per-RFQ DG authorization is required"), { code: "RFQ_NOT_AUTHORIZED" });
  }
  const { accessToken } = await accessTokenAndConnection();
  try {
    const sent = await outbound.sendMessage({ accessToken, message: internalTestMessage() });
    const log = await store.markOutboundSent(INTERNAL_SEND_TEST_RFQ.id, sent.id);
    return { ...log, gmailMessageId: sent.id, threadId: sent.threadId, internalTest: true, externalSupplier: false };
  } catch (error) {
    await store.markOutboundFailed(INTERNAL_SEND_TEST_RFQ.id, error.message);
    throw error;
  }
}
...
async function sendAuthorizedSupplierRfq({ workflowId, rfqId }) {
  ...
  if (rfq.supplier === "Hilti" && !attachments.length) {
    throw Object.assign(new Error("The verified Hilti RFQ PDF is unavailable"), { code: "RFQ_ATTACHMENT_MISSING" });
  }
  const { accessToken } = await accessTokenAndConnection();
  try {
    ...
    const supplierCycle = {
      ...
      rfqs: workflow.dossier.supplierCycle.rfqs.map((item) => item.id === rfq.id
        ? { ...item, status: "RFQ ENVOYEE", emailSent: true, sentAt, gmailMessageId: sent.id }
        : item)
    };
    ...
    await orchestratorStore.updateDossier(
      ...
      { rfqId: rfq.id, supplier: rfq.supplier, recipient: rfq.contact.email, lotNumber: rfq.lotNumber, gmailMessageId: sent.id },
      "supplier-ai"
    );
    return sendLog;
  } catch (error) {
...
async function syncSupplierReplies() {
  ...
  const rfqs = [...(workflow?.dossier?.supplierCycle?.rfqs || []), INTERNAL_ROUTING_TEST_RFQ, INTERNAL_SEND_TEST_RFQ];
  const query = encodeURIComponent('newer_than:30d {subject:"ITB/2026/62389" subject:"NEXUS-RFQ-ITB2026-62389" subject:"NEXUS-RFQ-ROUTING-TEST" subject:"NEXUS-RFQ-SEND-TEST"}');
  ...
  return { mailbox: cfg.mailbox, checked: ids.length, archived, matched, lastSyncAt: status?.last_sync_at || null, identities: identityStatus, sendPerformed: false };
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === "authorize-internal-send-test") {
      if (req.method !== "POST") return json(res, 405, { ok: false, error: "Methode non autorisee" });
      return json(res, 200, { ok: true, data: await authorizeInternalSendTest(session.email) });
    }
    if (action === "send-internal-test") {
      if (req.method !== "POST") return json(res, 405, { ok: false, error: "Methode non autorisee" });
      return json(res, 200, { ok: true, data: await sendInternalTest() });
    }
    ...
    if (action === "internal-send-test-status") {
      const authorization = await store.outboundAuthorization(INTERNAL_SEND_TEST_RFQ.id);
      const inbound = await store.latestInboundByRfq(INTERNAL_SEND_TEST_RFQ.id);
      let sent = null;
      if (authorization?.provider_message_id) {
        const { accessToken } = await accessTokenAndConnection();
        const source = await gmailJson(`messages/${encodeURIComponent(authorization.provider_message_id)}?format=full`, accessToken);
        const normalized = gmail.normalizeGmailMessage(source);
        sent = { gmailMessageId: normalized.gmailMessageId, from: normalized.from, to: normalized.to, replyTo: normalized.replyTo, subject: normalized.subject };
      }
      return json(res, 200, { ok: true, data: { authorization, inbound, sent } });
    }
    let connection = null;
