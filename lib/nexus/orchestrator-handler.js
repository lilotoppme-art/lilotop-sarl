...
const { AGENTS, applyDecision, attachOfficialSources, authorizeUnopsSupplierRfq, prepareUnopsSupplierCycle, recordManualEoiSubmission, recordUnopsSupplierQuotation, refreshVaultControl, resumeWorkflow, uploadOfficialSource } = require("./orchestrator-service");
const { parseUploadRequest } = require("./tender-response-documents");
...
function safeError(error) {
  if (["DOCUMENT_PARSE_ERROR", "UNSUPPORTED_DOCUMENT", "UPLOAD_TOO_LARGE"].includes(error.code)) {
    return { status: error.code === "UPLOAD_TOO_LARGE" ? 413 : 400, code: error.code, error: error.message };
  }
...
async function post(req, res, action, session) {
  ...
  if (action === "attach-official-sources") {
    return json(res, 200, {
      ok: true,
      data: await attachOfficialSources(body.id, body, session.email)
    });
  }
  ...
  if (action === "prepare-unops-supplier-cycle") {
    return json(res, 200, {
      ok: true,
      data: await prepareUnopsSupplierCycle(body.id, session.email)
    });
  }
  if (action === "record-unops-supplier-quotation") {
    return json(res, 200, {
      ok: true,
      data: await recordUnopsSupplierQuotation(body.id, body.quotation || {}, session.email)
    });
  }
  if (action === "authorize-unops-supplier-rfq") {
    return json(res, 200, {
      ok: true,
      data: await authorizeUnopsSupplierRfq(body.id, body.rfqId, session.email)
    });
  }
  return json(res, 404, { ok: false, error: "Action inconnue" });
}
...
module.exports = async function orchestratorHandler(req, res) {
  ...
  try {
    ...
    if (req.method === "POST" && action === "upload-official-source") {
      const { fields, file } = await parseUploadRequest(req);
      return json(res, 201, {
        ok: true,
        data: await uploadOfficialSource(fields.id, fields.sourceUrl, file, session.email)
      });
    }
