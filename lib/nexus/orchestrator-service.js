...
function buildUnecaSubmissionReview(dossier, credential, organizationChartDocument = null) {
  ...
  const conditions = [
    {
      key: "ungm-registration",
      title: "Inscription UNGM et rattachement au Secretariat de l'ONU",
      daoText: '"not fully registered in UNGM"',
      page: "Page 2; rappel pages 3-4",
    ...
    {
      key: "ungm-profile-current",
      title: "Profil UNGM maintenu a jour",
    ...
    {
      key: "vendor-response-form",
      title: "Vendor Response Form transmis avant l'echeance",
      daoText: '"Vendor Response Form ... electronically"',
      page: "Pages 2-3",
      status: "BROUILLON PRE-REMPLI - NON SOUMIS",
      proof: "Brouillon NEXUS prepare; aucune transmission UNGM effectuee",
      action: "Valider les champs puis utiliser manuellement Express interest avant le 14 aout 2026",
      completed: false
    },
    {
      key: "eligibility-a-f",
  ...
  const vendorResponseForm = {
    status: "BROUILLON - A VALIDER PAR LE DG",
    submissionMode: "UNGM - bouton Express interest; aucune soumission automatique",
    fields: [
      ["TO", "Solomon Gebreegziabher"],
      ["EOI Number", "EOIUNECA24536"],
      ["FROM", "LILOTOP SARL"],
      ...
      ["Signature / Date", "A VALIDER ET SIGNER PAR LE DG"]
    ],
    knownAddresses: [
...
async function refreshVaultControl(id, actorEmail) {
  ...
  const uneceSubmissionReview = uneceWorkflow
    ...
    : null;
  if (uneceSubmissionReview) rebuilt = { ...rebuilt, uneceSubmissionReview };
  ...
  const updated = await store.updateDossier(
    workflow.id,
    { ...dossier, vaultDocuments, tenderResponse, finalValidation, uneceSubmissionReview: uneceSubmissionReview || dossier.uneceSubmissionReview },
    actorEmail,
