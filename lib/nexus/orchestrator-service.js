...
function buildUnecaSubmissionReview(dossier, credential, organizationChartDocument = null) {
  ...
  const eligibility = [
    ["A", "The company, its parent entities, subsidiaries or affiliates are not listed on, or associated with a company or individual listed on, the United Nations Security Council Consolidated Sanctions List or the IIC Oil-for-Food List, unless disclosed in writing to the UN Procurement Division.", "Proposition OUI sous reserve de la confirmation DG de la liste complete des entites liees; la recherche publique n'a identifie aucun resultat exact LILOTOP SARL", "A CONFIRMER"],
    ["B", "The company, its parent entities, subsidiaries or affiliates are not currently removed or suspended by the United Nations or any other UN organization, including the World Bank.", "Proposition OUI sous reserve de la confirmation du statut dans le compte UNGM et par le DG", "A CONFIRMER"],
    ["C", "The company, its parent entities, subsidiaries or affiliates are not under formal investigation and have not been sanctioned within the preceding three years by any national authority of a United Nations Member State for proscribed practices, including corruption, fraud, coercion, collusion, obstruction or other unethical practice.", "Proposition OUI sous reserve d'une declaration explicite de la Direction Generale", "A CONFIRMER"],
    ["D", "The company has not declared bankruptcy, is not involved in bankruptcy or receivership proceedings, and there is no judgment or pending legal action that could impair its operations in the foreseeable future.", "Proposition OUI au regard des documents juridiques et fiscaux actifs dans le Coffre, sous reserve de confirmation juridique par le DG", "A CONFIRMER"],
    ["E", "The company does not employ, or anticipate employing, any person who is or was a United Nations staff member within the last year where that person had prior professional dealings with the vendor during the last three years of UN service, in accordance with ST/SGB/2006/15.", "Proposition OUI sous reserve de confirmation par la Direction Generale qu'aucune situation incompatible n'existe", "A CONFIRMER"],
    ["F", "The company undertakes not to engage in proscribed practices in connection with the United Nations or any other party and to conduct business in a manner that avoids financial, operational, reputational or other undue risk to the United Nations.", "Proposition OUI; l'engagement peut etre pris mais doit etre valide et signe par le DG", "A CONFIRMER"]
  ].map(([key, requirement, proof, status]) => ({
    key,
    requirement,
    response: "OUI - PROPOSITION A VALIDER PAR LE DG",
    proof,
    status,
  ...
  return {
    ...
    ungmComparison: {
      ...
      divergences: [],
      note: "Aucune divergence verifiable automatiquement: le profil fournisseur detaille UNGM est prive. Comparer uniquement apres ouverture de la session UNGM autorisee."
    }
  };
}
...
function buildUnecaEoiSubmission(review) {
  ...
  const dgFields = [
    "Confirmer le statut Registered au niveau Basic avec le Secretariat de l'ONU dans UNGM",
    "Confirmer individuellement les declarations A, B, C, D, E et F",
    "Verifier uniquement les divergences eventuelles entre le profil UNGM prive et les donnees NEXUS",
    "Cliquer manuellement sur Express interest apres validation"
  ];
  const letter = [
  ...
  return {
    ...
    rejectionRisk: "AUCUN BLOCAGE TECHNIQUE; CONFIRMATION BASIC ET DECLARATIONS A-F REQUISES AVANT L'ACTION UNGM",
    recommendation: "EXPRESS INTEREST APRES CONFIRMATION DU STATUT BASIC ET DES DECLARATIONS A-F",
    readyItems: [
      ...
      "Lettre EOI, declarations A-F et dossier interne PDF/ZIP prepares",
      "Aucun document, prix, quantite ou specification detaillee requis a cette etape"
    ],
    dgValidationItems: dgFields.slice(0, 3),
    blockingItem: "Validation humaine du statut Basic et des declarations A-F; aucune autre information interne connue ne bloque le dossier.",
    expressInterestPayload: [
      ...
      "Codes UNSPSC: 39000000, 39100000, 39110000",
