...
function buildFinalValidation(dossier, tenderResponse, comparison) {
  ...
  const supplierRfqs = (dossier.rfqs || []).slice(0, MAX_SOURCED_PRODUCTS).map((rfq) => {
    ...
    const coordinatesVerified = Boolean(
      (supplier.commercialEmail || supplier.phone)
      && (supplier.website || supplier.sourceUrl || supplier.evidence)
    );
    return {
      id: rfq.id,
      supplier: supplier.name || "Fournisseur à confirmer",
      product: rfq.product || rfq.description || "Produit à confirmer",
      commercialEmail: supplier.commercialEmail || null,
      phone: supplier.phone || null,
      source: supplier.website || supplier.sourceUrl || null,
      coordinatesVerified,
      specifications: rfq.description || "Spécifications à confirmer",
      quantity: rfq.quantity || "À confirmer",
      desiredDelivery: rfq.desiredDelivery || "À confirmer",
      responseDeadline: rfq.responseDeadline || "À définir par la DG",
