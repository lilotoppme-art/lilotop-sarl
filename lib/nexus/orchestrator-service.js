...
function buildGoNoGoAssessment(opportunity, analysis, vaultDocuments = [], now = new Date()) {
  const deadline = deadlineIntake(analysis.deadline || opportunity.deadlineAt, now);
  const requiredDocuments = analysis.requiredDocuments || [];
  const control = buildDocumentControl(requiredDocuments, vaultDocuments, []);
  const missing = control.rows.filter((item) => item.status !== "available");
  const combined = [
    ...requiredDocuments,
    ...(analysis.requirements || []),
    ...(analysis.risks || [])
  ].join(" ");
  const partnerAllowed = /partenaire|partner|sous[- ]trait|subcontract|consortium|joint venture|\bjv\b/i.test(combined);
  const critical = missing.filter((item) => /exp[eé]rience|contrat similaire|bilan|chiffre d.affaires|garantie|assurance|certification|agr[eé]ment|visite|cv|personnel|technicien/i.test(item.document));
  let decision = analysis.opportunityScore >= 85 && !missing.length ? "A+" : "A";
  let label = decision === "A+" ? "A+ — GO" : "A — GO";
  if (deadline.expired || analysis.lilotopFit === false) {
    decision = "NO-GO";
    label = "NO-GO";
  } else if (critical.length || partnerAllowed) {
    decision = "B";
    label = "B — GO SOUS CONDITION";
  }
  return {
    decision,
    label,
    ...
    availableDocumentCount: control.availableDocuments.length,
    missingDocuments: control.missingDocuments,
    expiredDocuments: control.expiredDocuments,
    criticalGaps: critical.map((item) => item.document),
    stopCostlyWorkflow: decision === "NO-GO",
...
async function analyzeStep(workflow, actorEmail) {
  ...
  const [analysis, vaultDocuments] = await Promise.all([runAction(workflow, actorEmail, {
  ...
  const goNoGo = await runAction(workflow, actorEmail, {
    agentKey: "tender-ai",
    actionKey: "go-no-go-first",
    label: "Contrôle GO / NO-GO avant sourcing",
    input: { opportunityId: workflow.opportunityId },
    output: (result) => ({ ...result, externalAction: false })
  }, () => buildGoNoGoAssessment(dossier.opportunity, analysis, vaultDocuments));
  ...
  const nextDossier = {
    ...
    parallelExecution: {
      ...
      maxActiveDossiers: 3,
      workstreams: [
        { agent: "commercial-ai", status: "completed", task: "qualification économique" },
        { agent: "document-vault", status: "completed", task: "contrôle documents et expériences" },
        { agent: "tender-response-ai", status: "ready", task: "formulaires et conformité" },
        { agent: "supplier-ai", status: analysis.products.length ? "ready" : "not-required", task: "sourcing après GO" }
      ],
      externalActionsAutomatic: false
    },
    pipelineStatus: goNoGo.stopCostlyWorkflow ? "rejected" : "analyzed",
    sourceIndex: 0
  };
  if (goNoGo.stopCostlyWorkflow) {
    return store.advanceWorkflow(workflow.id, {
      status: "completed",
...
async function authorizeUnopsSupplierRfq(id, rfqId, actorEmail) {
  ...
  const authorizedRfq = supplierCycle.rfqs.find((item) => item.id === rfqId);
  await gmailStore.authorizeOutbound({
    rfqId: authorizedRfq.id,
    supplier: authorizedRfq.supplier,
    recipient: authorizedRfq.contact.email,
    lotNumber: authorizedRfq.lotNumber,
    authorizedBy: actorEmail,
    metadata: { workflowId: workflow.id, trackingId: authorizedRfq.trackingId, internalTest: false }
  });
  const finalValidation = {
    ...(dossier.finalValidation || {}),
    supplierCycle,
...
async function resumeWorkflow(id, actorEmail) {
  ...
  if (workflow.status !== "running" && await store.countIntensiveWorkflows(workflow.id) >= 3) {
    throw Object.assign(new Error("Trois appels d'offres sont déjà en traitement intensif."), { code: "VALIDATION_ERROR" });
  }
