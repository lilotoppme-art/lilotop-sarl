...
function classifySupplierResponse(message) {
  ...
  if (/mailer-daemon|postmaster|no-?reply|do-?not-?reply/.test(sender)
    || /out of office|automatic reply|auto.?reply|delivery status|undeliverable|non.?delivery|absence du bureau/.test(combined)) {
  ...
  } else if (/thank you for your (?:email|enquiry|inquiry|request)|received|acknowledge|well received|accus[eé] de r[eé]ception/.test(combined)) {
...
module.exports = { GMAIL_READONLY, GMAIL_SEND, authorizationUrl, classifySupplierResponse, config, correlationScore, extractQuotation, matchSupplierReply, normalizeGmailMessage, processSupplierReply, readiness, stateToken, verifyState };
