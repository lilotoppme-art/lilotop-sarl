(() => {
  ...
  async function openDetail(id) {
    ...
    try {
      ...
      const analysisRisks = Array.isArray(item.ai_analysis?.risks) ? item.ai_analysis.risks : [];
      const intake = item.raw_data?.intake || {};
      document.querySelector("[data-detail-content]").innerHTML = `<div class="detail-grid"><div><span>Reference</span><strong>${esc(intake.reference || item.external_id || "Non precisee")}</strong></div><div><span>Organisation</span><strong>${esc(item.organization || "Non precisee")}</strong></div><div><span>Pays / ville</span><strong>${esc([item.country, intake.city].filter(Boolean).join(" / ") || "Non precise")}</strong></div><div><span>Score</span><strong>${item.score}/100</strong></div><div><span>Echeance officielle</span><strong>${esc(date(item.deadline_at, true))}</strong></div><div><span>Echeance interne LILOTOP</span><strong>${esc(date(intake.internalDeadline, true))}</strong></div><div><span>Temps restant</span><strong>${esc(intake.remainingHours === null || intake.remainingHours === undefined ? "Non calcule" : `${intake.remainingHours} h`)}</strong></div><div><span>Statut workflow</span><strong>${esc(intake.workflowStatus || "Non initialise")}</strong></div><div><span>Dossier source</span><strong>${esc(intake.documentRetrievalStatus || "Non controle")}</strong></div><div><span>Mode d'analyse</span><strong>${esc(item.analysis_mode || "non precise")}</strong></div></div><p>${esc(item.ai_summary || item.description || "Aucun resume disponible.")}</p>${analysisRisks.length ? `<p><strong>Diagnostic :</strong> ${esc(analysisRisks.join(" | "))}</p>` : ""}${item.source_url ? `<a class="button ghost" href="${esc(item.source_url)}" target="_blank" rel="noopener noreferrer">Ouvrir la source</a>` : ""}`;
