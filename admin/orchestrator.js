...
function supplierCycleMarkup(cycle, workflow) {
  ...
  return `<article id="supplier-rfq-cycle" class="validation-rfqs supplier-cycle">
    ...
    ${cycle.responseStats ? `<div class="supplier-cycle-stats supplier-response-stats">
      <div><span>Cotations exploitables</span><strong>${escapeHtml(cycle.responseStats.exploitableQuotations || 0)}</strong></div>
      <div><span>Accuses de reception</span><strong>${escapeHtml(cycle.responseStats.acknowledgements || 0)}</strong></div>
      <div><span>Demandes de precision</span><strong>${escapeHtml(cycle.responseStats.clarificationRequests || 0)}</strong></div>
      <div><span>Refus</span><strong>${escapeHtml(cycle.responseStats.refusals || 0)}</strong></div>
      <div><span>Redirections</span><strong>${escapeHtml(cycle.responseStats.redirects || 0)}</strong></div>
      <div><span>Messages automatiques</span><strong>${escapeHtml(cycle.responseStats.automaticMessages || 0)}</strong></div>
      <div><span>RFQ sans bounce detecte</span><strong>${escapeHtml(cycle.responseStats.effectivelyDelivered ?? cycle.counts.delivered ?? 0)}</strong></div>
      ...
      <div><span>Sans reponse</span><strong>${escapeHtml(cycle.responseStats.withoutResponse || 0)}</strong></div>
    </div>` : ""}
