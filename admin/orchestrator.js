...
function supplierCycleMarkup(cycle, workflow) {
  ...
  return `<article id="supplier-rfq-cycle" class="validation-rfqs supplier-cycle">
    ...
    <div class="supplier-cycle-stats">
      ...
      <div><span>Tentatives d'envoi</span><strong>${escapeHtml(cycle.responseStats?.sendAttempts ?? cycle.counts.sent)}</strong></div>
      <div><span>RFQ actives</span><strong>${escapeHtml(cycle.responseStats?.activeRfqs ?? cycle.counts.active ?? cycle.counts.sent)}</strong></div>
      <div><span>Cotations manquantes</span><strong>${escapeHtml(cycle.counts.missing)}</strong></div>
    </div>
    ${cycle.responseStats ? `<div class="supplier-cycle-stats supplier-response-stats">
      <div><span>RFQ uniques</span><strong>${escapeHtml(cycle.responseStats.uniqueRfqs ?? cycle.counts.uniqueRfqs ?? 0)}</strong></div>
      <div><span>Sent</span><strong>${escapeHtml(cycle.responseStats.sent ?? cycle.counts.sent ?? 0)}</strong></div>
      <div><span>No bounce detected</span><strong>${escapeHtml(cycle.responseStats.noBounceDetected ?? cycle.counts.delivered ?? 0)}</strong></div>
      <div><span>Delivery failed</span><strong>${escapeHtml(cycle.responseStats.deliveryFailed ?? cycle.counts.deliveryFailed ?? 0)}</strong></div>
      <div><span>Acknowledged</span><strong>${escapeHtml(cycle.responseStats.acknowledged ?? cycle.responseStats.acknowledgements ?? 0)}</strong></div>
      <div><span>Responses received</span><strong>${escapeHtml(cycle.responseStats.responsesReceived ?? 0)}</strong></div>
      <div><span>Quotations received</span><strong>${escapeHtml(cycle.responseStats.quotationsReceived ?? 0)}</strong></div>
      <div><span>Quotations exploitables</span><strong>${escapeHtml(cycle.responseStats.exploitableQuotations || 0)}</strong></div>
    </div>` : ""}
    ...
    <div class="responsive-table"><table class="rfq-review-table"><thead><tr><th>Fournisseur</th><th>Lot</th><th>Lignes</th><th>Confirmee</th><th>Probable</th><th>Rejetee</th><th>Contact direct</th><th>RFQ PDF</th><th>Priorite</th><th>Recommandation</th><th>Date reponse</th><th>Statut envoi</th><th>Action</th></tr></thead><tbody>
      ${(cycle.rfqs || []).map((rfq) => `<tr><td>${escapeHtml(rfq.supplier)}</td><td>${escapeHtml(rfq.lotNumber)}</td><td>${escapeHtml((rfq.products || []).length)}</td><td>${escapeHtml(rfq.coverageCounts?.confirmed || 0)}</td><td>${escapeHtml(rfq.coverageCounts?.probable || 0)}</td><td>${escapeHtml(rfq.coverageCounts?.rejected || 0)}</td><td>${rfq.contact?.verified ? "OUI" : "NON"}</td><td>${rfq.rfqPdfReady ? "OUI" : "NON"}</td><td>${escapeHtml(rfq.priority || "C")}</td><td>${escapeHtml(rfq.sendRecommendation || "NON")}</td><td>${escapeHtml(rfq.responseDeadlineLabel || rfq.responseDeadline || "A FIXER")}</td><td>${escapeHtml(rfq.trackingStatus || (rfq.sentAt ? "SENT" : "NON AUTORISE"))}</td><td><div class="rfq-card-actions"><button class="button button-secondary" type="button" data-toggle-rfq="${escapeHtml(rfq.id)}">VOIR RFQ</button><button class="button button-secondary" type="button" data-edit-rfq="${escapeHtml(rfq.id)}">MODIFIER</button>${rfq.readyForDgReview && rfq.sendRecommendation === "OUI" && !rfq.sentAt ? `<button class="button button-primary" type="button" data-request-rfq-authorization="${escapeHtml(rfq.id)}" data-rfq-pilot="${rfq.id === "UNOPS-62389-L1-HILTI" ? "true" : "false"}" data-rfq-supplier="${escapeHtml(rfq.supplier)}" data-rfq-recipient="${escapeHtml(rfq.contact.email || rfq.contact.contactForm)}" data-rfq-lot="${escapeHtml(rfq.lotNumber)}" data-rfq-lines="${escapeHtml((rfq.products || []).length)}" data-rfq-attachments="${escapeHtml((rfq.attachments || []).join(", "))}" data-rfq-deadline="${escapeHtml(rfq.responseDeadlineLabel || rfq.responseDeadline)}">AUTORISER</button>` : rfq.sentAt ? "SUIVI UNIQUEMENT" : "ENVOI NON AUTORISE"}</div></td></tr>`).join("")}
    </tbody></table></div>
    ${(cycle.rfqs || []).map((rfq) => `<section class="supplier-rfq-card" data-rfq-card="${escapeHtml(rfq.id)}">
      <div class="section-heading-inline"><div><h4>${escapeHtml(rfq.supplier)} - Lot ${escapeHtml(rfq.lotNumber)}</h4><p>${escapeHtml(rfq.lotTitle)}</p></div><span class="status status-paused">${escapeHtml(rfq.trackingStatus || (rfq.sentAt ? "SENT" : rfq.status))}</span></div>
      <div class="rfq-meta-grid">
      ...
      <div class="rfq-card-actions">
        ...
        ${rfq.authorizedAt && !rfq.sentAt ? `<button class="button button-primary" type="button" data-send-authorized-rfq="${escapeHtml(rfq.id)}">ENVOYER LA RFQ AUTORISEE</button>` : rfq.sentAt ? "" : `<button class="button button-primary" type="button"
          ...
          ${rfq.readyForDgReview && rfq.sendRecommendation === "OUI" ? "" : "disabled"}>${rfq.sendRecommendation === "OUI" ? "AUTORISER L'ENVOI" : "ENVOI NON RECOMMANDE"}</button>`}
      </div>
  ...
  <article class="validation-rfqs supplier-cycle"><h3>REPONSES FOURNISSEURS / COTATIONS RECUES</h3>
    ${(cycle.responses || []).length ? `<div class="responsive-table"><table><thead><tr><th>Fournisseur</th><th>RFQ</th><th>Lot / lignes</th><th>Date de reponse</th><th>Expediteur</th><th>Type</th><th>Pieces jointes</th><th>Prix</th><th>Conformite</th><th>Action recommandee</th></tr></thead><tbody>${cycle.responses.map((quote) => `<tr><td>${escapeHtml(quote.supplier)}</td><td>${escapeHtml(quote.rfqId)}<br><small>Thread : ${escapeHtml(quote.gmailThreadId || "Non disponible")}</small></td><td>${escapeHtml(quote.lotNumber)} / ${escapeHtml((quote.lineNumbers || []).join(", "))}</td><td>${escapeHtml(quote.receivedAt)}</td><td>${escapeHtml(quote.sender)}</td><td>${escapeHtml(quote.responseType)}</td><td>${escapeHtml(quote.attachmentCount || 0)}${(quote.attachments || []).map((item) => `<br><small>${escapeHtml(item.filename)}</small>`).join("")}</td><td>${quote.quotationExploitable ? escapeHtml(money(quote.totalPrice, quote.currency)) : "NON"}</td><td>${escapeHtml(quote.technicalCompliance || "INFORMATION INSUFFISANTE")}</td><td>${escapeHtml(quote.recommendedAction)}</td></tr>`).join("")}</tbody></table></div>` : "<p>Aucune reponse fournisseur reelle recue.</p>"}
  </article>
